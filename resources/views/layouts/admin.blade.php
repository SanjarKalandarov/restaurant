...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <link rel="stylesheet" href="sha512-R+xPS2VPCAFvLRy+I4PgbwkWjw1z5B5gNDYgJN5LfzV4gGNeRQyVrY7Uk59rX+c8tzz63j8DeZPLqmXvBxj8pA==">
        <!-- Scripts -->
        ...
        <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-slate-100 md:w-64 dark:text-gray-200 dark:bg-gray-800" x-data="{ open: false }">
            ...
            <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
                ...
                <x-admin-nav-link :href="route('admin.table.index')" :active="request()->routeIs('admin.table.index')">
                    {{ __('Table') }}
                </x-admin-nav-link> <x-admin-nav-link :href="route('admin.roservation.index')" :active="request()->routeIs('admin.roservation.index')">
                    {{ __('Roservation') }}
                </x-admin-nav-link>
                 ...
                 <div @click.away="open = false" class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg">
                        <div class="px-2 py-2 bg-black rounded-md shadow dark:bg-gray-700">
    ...
    <script >function  menu(){
        alert('Menu ochirisha ichonchingiz komilmi');
        }</script>
    </body>
</html>
